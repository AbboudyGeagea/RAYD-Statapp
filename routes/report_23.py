...
def get_report_config(form):
    # Your exact base SQL from the database
    ...
    # Apply the type casting to your joins to ensure 'text = bigint' errors don't happen
    if base_sql:
...
def report_23():
    ...
    if run_report and base_sql:
        ...
        # 1. Main Aggregation (Amending with AND)
        agg_sql = text(f"""
            {cte_base} 
            SELECT modality, patient_class, age_at_exam, study_date 
            FROM base_data 
            WHERE 1=1 
            AND study_date BETWEEN :s AND :e
        """)
        
        # 2. Age Bucket + Study Description (Amending with AND)
        # We need study_description - assuming it's in s. (etl_didb_studies)
        # If your base SQL doesn't have study_description, we'll use proc_id as a fallback
        demo_sql = text(f"""
            {cte_base}
            SELECT 
                CASE 
        ...
        try:
            ...
            if not df_agg.empty:
                ...
                # Convert age_at_exam to numeric just in case it's a string
                ...
                age_order = ['[0-1 month]', '[1 month - 1 year]', '[1-12 years]', '[13-18]', '[19-35]', '[36-64]', '[65+]']
                
                if not df_demo.empty:
                    top_5 = df_demo.groupby('description')['cnt'].sum().nlargest(5).index.tolist()
                    df_demo['display_desc'] = df_demo['description'].apply(lambda x: x if x in top_5 else 'Others')
                    
                    pivot = df_demo.pivot_table(index='age_bucket', columns='display_desc', values='cnt', aggfunc='sum').fillna(0)
                    ...
                    chart_json = {
                        ...
                        "age_desc_stack": {
                            "labels": age_order,
                            "series": [{"name": col, "type": "bar", "stack": "total", "data": pivot[col].tolist()} for col in pivot.columns]
                        }
                    }
        except Exception as e:
            print(f"Error executing report: {e}")

    return render_template("report_23.html", display_start=start_date, display_end=end_date, metrics=metrics, chart_json=chart_json, run_report=run_report)
