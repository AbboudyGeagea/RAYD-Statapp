...
logger = logging.getLogger("CD_PRINT")
    ...
    try:
        ...
        if search:
            base_filters.append("""(
                ...
                OR pv.fallback_id     ILIKE :search
            )""")
        ...
        logger.error(f"cd_print_log API error: {e}", exc_info=True)
        db.session.rollback()
        return jsonify({"error": str(e), "rows": [], "total": 0, "page": 1, "per_page": 50}), 500
...
def trigger_cd_sync():
    ...
    try:
        ...
        logger.error(f"CD surf sync error: {e}", exc_info=True)
