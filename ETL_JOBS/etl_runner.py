...
Execution phases (in order):
  ...
  Phase 17 — RIS Worklist Status Events (LAUMC: WORKLIST_STATUS_HISTORY ⋈ SITE_WORKLIST,
             three sub-steps sharing the same table/source — arrivals [status_key=60,
             "Arrived"] -> std_worklist_arrivals, real patient-arrival timestamps from
             RIS's own status-transition audit log, not the still-empty
             hl7_orders.arrived_at (depends on R2I's live HL7 ORM feed); exam_done
             [status_key=100, "Exam Done"] -> std_worklist_exam_done, a RIS-side "exam
  ...
  Phase 18 — RIS Device Schedule Resolution (LAUMC: RIS SCHEDULE_TEMPLATE_VERSION ->
             ...
             the gap Phase 15/migration 0067 left open — then computes
             std_device_weekly_availability [simple Available-minutes-per-weekday per
             device, RAYD day-of-week convention, via an interval sweep resolving
             overlapping/superseded schedule rows by source_last_updated recency;
             TRUNCATE+rebuilt every pass since schedules change ~weekly]. Sub-steps 2-3 are
             pure Postgres, no Oracle needed; skipped unless a RIS db_params source is
             configured)
...
_PHASE_LABELS = {
    ...
    '10': ('RIS Catalog',        'Oracle RIS: MODALITY + SPS_CODE + MODALITY_SCHEDULE + ORDERING_ORGANIZATION -> aetitle_modality_map / procedure_duration_map (incl. modality fill-in) / std_ordering_organizations — light (LAUMC)'),
    ...
    try:
        ...
        if _confirm_phase(10):
            ...
            if not _ris_ok:
                ...
                logger.info("📋 Phase 10: RIS Catalog (Modality + Procedure codes + Modality Schedule + Ordering Orgs)")
                try:
        ...
        if _confirm_phase(11):
            ...
            if not _ris_ok:
                logger.info(
                    f"⏭  Phase 11 skipped — no RIS source configured. Add a db_params entry "
                    f"named '{ris_src}' (or set RAYD_RIS_SOURCE) pointing at the RIS Oracle."
                )
            else:
        ...
        if _confirm_phase(17):
            ...
            if not _ris_ok:
                ...
                logger.info("📋 Phase 17: RIS Worklist Status Events")
                _phase17_substeps = [
                ...
                for _name, _fn in _phase17_substeps:
                    try:
                        ...
                        _phase17_failed.append(_name)
                        logger.error(f"🛑 Phase 17 sub-step '{_name}' failed — continuing to next sub-step: {_e}", exc_info=True)
                if _phase17_failed:
                    _phase_failures.append(f"17 (RIS Worklist Status Events: {', '.join(_phase17_failed)})")
