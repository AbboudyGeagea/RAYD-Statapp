...
3. **exclude_from_stats** — AEs flagged in `aetitle_modality_map.exclude_from_stats = TRUE` must be excluded from all stats queries. Current exclusion: `NonDICOMAAgent` (PACS internal non-DICOM document importer — produces mixed-modality noise). Pattern when `m` is already joined: `AND COALESCE(m.exclude_from_stats, FALSE) = FALSE`. Pattern without the join: `AND s.storing_ae NOT IN (SELECT aetitle FROM aetitle_modality_map WHERE exclude_from_stats = TRUE)`.
