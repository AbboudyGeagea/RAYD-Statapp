...
etl_didb_studies
  ...
  original_storing_ae TEXT, accession_number TEXT, number_of_study_images INT,
...
### Configuration
```
aetitle_modality_map    — one row PER MODALITY (aetitle = modality, see convention 3)
  id SERIAL PK, aetitle VARCHAR NOT NULL, modality VARCHAR NOT NULL,
  daily_capacity_minutes INT DEFAULT 480 (summed across the site's devices of that type)
...
device_exceptions       — per-day capacity overrides per modality (aetitle holds the modality)
  id, aetitle, exception_date DATE, actual_opening_minutes INT, reason VARCHAR

device_weekly_schedule  — standard weekly schedule per modality (aetitle holds the modality)
...
3. **Modality-only site (migration 0063)** — the PACS destroys device AE identity (a scheduled "Update DICOM tags" job overwrites `original_storing_ae` with `NonDicomAgent` for ~24% of studies; unrecoverable — station_name, audit trail, and AUTOROUTER tables were all ruled out). Therefore `etl_didb_studies.original_storing_ae` holds the **modality group** (CT, MR, US, OT…), normalized by ETL Phase 2b on every sync. `aetitle_modality_map` has exactly one row per modality with `aetitle = modality`; capacity and schedules are per modality (sum of the site's devices of that type). Never reintroduce per-device AE logic or AE-based exclusions.
4. **Nothing excluded** — `exclude_from_stats` is FALSE for every map row. Existing `COALESCE(m.exclude_from_stats, FALSE) = FALSE` filters in queries are harmless no-ops; do not add new exclusions. The only filtered value is modality `SR` (convention 2).
5. **Modality source** — `COALESCE(m.modality, s.study_modality)` still works everywhere; both sides now hold the same modality group value.
6. **Expensive CTEs** — `etl_orders` scans with `MODE() WITHIN GROUP` must use `WITH ... AS MATERIALIZED`.
7. **hl7_oru_analysis** — written only by `nlp_worker/worker.py`; route handlers read it but never write it.
8. **Qwen prompt format** — ChatML: `<|im_start|>system\n...<|im_end|>\n<|im_start|>user\n...<|im_end|>\n<|im_start|>assistant\n`
9. **Password encryption** — Oracle and external DB passwords are encrypted via `utils/crypto.py` using `SECRET_KEY`.
